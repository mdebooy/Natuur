...
ALTER TABLE NATUUR.FOTOS ALTER COLUMN FOTO_ID DROP DEFAULT;
ALTER TABLE NATUUR.FOTOS ALTER COLUMN FOTO_ID ADD GENERATED ALWAYS AS IDENTITY;
SELECT SETVAL(PG_GET_SERIAL_SEQUENCE('natuur.fotos', 'foto_id'), COALESCE(MAX(FOTO_ID), 1))
FROM NATUUR.FOTOS;
DROP SEQUENCE NATUUR.SEQ_FOTOS;

ALTER TABLE NATUUR.GEBIEDEN ALTER COLUMN GEBIED_ID DROP DEFAULT;
ALTER TABLE NATUUR.GEBIEDEN ALTER COLUMN GEBIED_ID ADD GENERATED ALWAYS AS IDENTITY;
SELECT SETVAL(PG_GET_SERIAL_SEQUENCE('natuur.gebieden', 'gebied_id'), COALESCE(MAX(GEBIED_ID), 1))
FROM NATUUR.GEBIEDEN;
DROP SEQUENCE NATUUR.SEQ_GEBIEDEN;

ALTER TABLE NATUUR.TAXA ALTER COLUMN TAXON_ID DROP DEFAULT;
ALTER TABLE NATUUR.TAXA ALTER COLUMN TAXON_ID ADD GENERATED ALWAYS AS IDENTITY;
SELECT SETVAL(PG_GET_SERIAL_SEQUENCE('natuur.taxa', 'taxon_id'), COALESCE(MAX(TAXON_ID), 1))
FROM NATUUR.TAXA;
DROP SEQUENCE NATUUR.SEQ_TAXA;

ALTER TABLE NATUUR.WAARNEMINGEN ALTER COLUMN WAARNEMING_ID DROP DEFAULT;
ALTER TABLE NATUUR.WAARNEMINGEN ALTER COLUMN WAARNEMING_ID ADD GENERATED ALWAYS AS IDENTITY;
SELECT SETVAL(PG_GET_SERIAL_SEQUENCE('natuur.waarnemingen', 'waarneming_id'), COALESCE(MAX(WAARNEMING_ID), 1))
FROM NATUUR.WAARNEMINGEN;
DROP SEQUENCE NATUUR.SEQ_WAARNEMINGEN;
